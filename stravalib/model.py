...
class BaseEntity(metaclass=abc.ABCMeta):
    ...
    def to_dict(self):
        """
        Create a dictionary based on loaded attributes in this object (will 
        not lazy load).

        Returns
        -------
        dict
            The dictionary created by this method will most likely not 
            match the dictionaries that are passed to the `from_dict()` method.  
            This intended for serializing for local storage debug/etc.
    ...
    def from_dict(self, d):
        ...
        Only defined attributes will be set; warnings will be logged for 
        invalid attributes.
        ...
        for (k, v) in d.items():
            ...
            if hasattr(self.__class__, k):
                self.log.debug(
                    'Setting attribute `{0}` [{1}] on entity {2} with' \
                    'value {3!r}'.format(
                        k,
                ...
                try:
                    ...
                    raise AttributeError(
                        'Could not find attribute `{0}` on entity {1},' 
                        'value: {2!r}.  (Original: {3!r})'.format(
                            k, self, v, x
                        )
                    )
            else:
...
class IdentifiableEntity(ResourceStateEntity):
    ...
    id = Attribute(
        int, (META, SUMMARY, DETAILED)
    )  #: INT The numeric ID for this entity.


class BoundEntity(BaseEntity):
    """
    Base class for entities that support lazy loading additional data using a 
    bound client.

    Attributes
    ----------
    bind_client : stravalib.client.Client
        The Client object that can be used to load related resources.
    """
    ...
    def __init__(self, bind_client=None, **kwargs):
        """
        Base entity initializer, which accepts a client parameter that creates 
        a "bound" entity which can perform additional lazy loading of content.

        Parameters
        ----------
        bind_client : stravalib.simple.Client (default None)
            The client instance to bind to this entity.
...
class LoadableEntity(BoundEntity, IdentifiableEntity):
    ...
    In theory these entities can be "expanded" by additional Client queries.  
    In practice this is not implemented, since usefulness is limited due to 
    resource-state limitations, etc.
    """
    ...
    def expand(self):
        ...
        """
        # This is a little harder now due to resource states, etc.
...
class Club(LoadableEntity):
    ...
    def activities(self):
        """
        An iterator of reverse-chronological :class:`stravalib.model.Activity` 
